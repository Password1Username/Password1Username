...
class MyPlayer(Inputs):
    ...
    def __init__(self, spritex, spritey, spritewidth, spriteheight, xpos=None, ypos=None):
        ...
        self.rects = {"up": rects_up, "down": rects_down, "left": rects_left, "right": rects_right}

    def setAnimation(self, spritesheet, name):
        if not self.x:
            self.x=self.rects[name][0][2]
            self.x_scaled=self.rects[name][0][2]

        if not self.width:
            self.width=self.rects[name][0][2]
            self.width_scaled=self.rects[name][0][2]
            self.collision_x1 = self.x
            self.collision_x2 = self.x+self.width
            self.collision_width = self.width

        if not self.y:
            self.y=self.rects[name][0][3]
            self.y_scaled=self.rects[name][0][3]

        if not self.height:
            self.height=self.rects[name][0][3]
            self.height_scaled=self.rects[name][0][3]
            self.collision_y1 = self.y
            self.collision_y2 = self.y+self.height
            self.collision_height = self.height

        temp=pyganim.getImagesFromSpriteSheet(spritesheet, rects=self.rects[name])
